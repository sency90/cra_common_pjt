...
inline void PrintChoiceSteerTypePage() {
    printf(CLEAR_SCREEN);
...
void RemoveEndNewLine(char *buf) {
    // 엔터 개행문자 제거
...
void ValidateInput(char *buf, int step) {
    // 숫자로 된 대답인지 확인
    char *checkNumber;
...
int ChangeInputTypeToInt(char *buf) noexcept {
    char *checkNumber;
    int answer = strtol(buf, &checkNumber, 10); // 문자열을 10진수로 변환
    return answer;
}
...
int main() {
    ...
    while(1) {
        ...
        GetInput(buf);
        RemoveEndNewLine(buf);

        if(WhenTypedExit(buf)) {
            ExitProgram();
        }

        try {
            ValidateInput(buf, step);
        }
        ...
        int answer = ChangeInputTypeToInt(buf);
